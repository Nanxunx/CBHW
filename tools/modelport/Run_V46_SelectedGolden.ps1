...
$StaticRows = $HeaderRows | Where-Object {
    $_.Has112 -match '(?i)^(true|1|yes)$' -and [string]::IsNullOrWhiteSpace($_.HeaderError) -and $_.ConversionPlan -eq "STATIC_GEOMETRY_SAFE"
} | Sort-Object { [int]$_.Vertices } | Select-Object -First $StaticLimit
$AnimatedRows = $HeaderRows | Where-Object {
    $_.Has112 -match '(?i)^(true|1|yes)$' -and [string]::IsNullOrWhiteSpace($_.HeaderError) -and $_.ConversionPlan -eq "ANIMATION_BASELINE"
...
foreach ($Sample in $Selection) {
    ...
    try {
        ...
        Get-ChildItem -LiteralPath $SrcDir -File -ErrorAction SilentlyContinue | Where-Object {
            $_.BaseName.StartsWith($Stem, [System.StringComparison]::OrdinalIgnoreCase) -and ($_.Extension -ieq ".skin" -or $_.Extension -ieq ".anim")
        ...
        if ($ExitCode -eq 0 -and (Test-Path -LiteralPath $OutM2 -PathType Leaf)) {
            ...
            $Status = "SKIP_LIGHT_REFERENCE_GATED"; $Message = "Light > 0 remains intentionally reference-gated"; Write-Host "  SKIP Light reference-gated" -ForegroundColor DarkYellow
        } else {
    ...
    $Results += [pscustomobject]@{
        Index=$Index; Category=$Sample.Category; RelativePath=$Rel; Animations=$Sample.Animations; Ribbons=$Sample.Ribbons; Particles=$Sample.Particles
        Status=$Status; ExitCode=$ExitCode; GeneratedBytes=$GeneratedBytes; Historical112Bytes=$GoldenBytes; Message=$Message
    }
}
...
$Summary = [ordered]@{
    ...
    converter=$Converter; converter_sha256=(Get-FileHash -LiteralPath $Converter -Algorithm SHA256).Hash
